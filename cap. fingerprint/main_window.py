...
class MainWindow(QMainWindow, Ui_FingerprintApp):
    DATABASE_PATH = os.path.expanduser("/home/ubuntu22/fingerprint/r307sensor/fingerprints.db")
    ...
    def update_messages_display(self):
        ...
        icon_mapping = {
            ...
            "detected": "👆", "search": "🔍", "error": "⚠️",
            "Waiting": "⏳", "Match": "✅", "No match": "❌",
            "Switched": "🔄", "Anti-spoof": "🛡️",
            "Deleting": "🗑️", "Enabling": "⚡", "Disabling": "⚡",
            "Enrollment": "📝", "Processing": "🔄", "Initializing": "⚙️",
            "Please": "👆", "Finger": "👆", "Place": "👆",
            "Remove": "👆", "Press": "👆", "Try": "🔄",
            "Total operation time": "⏱️"
        }
    ...
    def toggle_sensor_type(self):
        ...
        try:
            ...
            if self.current_sensor_type == "Capacitive":
                ...
                self.sensor = FingerprintSensor(port='/dev/ttyUSB0')
                self.append_to_results("✅ Optical sensor initialized successfully")
            else:
                self.current_sensor_type = "Capacitive"
                self.sensor = AnotherSensor(port='/dev/ttyUSB1')
